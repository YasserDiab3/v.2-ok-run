...
const DataManager = {
    ...
    save() {
        try {
            ...
            // ملاحظة: حفظ البيانات المحلية فقط - المزامنة مع Google Sheets تتم تلقائياً باستخدام autoSave
            return true;
        } catch (error) {
            Utils.safeError('❌ خطأ في حفظ البيانات المحلية:', error);
            if (error.message && error.message.includes('Maximum call stack')) {
                Notification.error('❌ فشل حفظ البيانات المحلية: البيانات كبيرة جداً. سيتم حفظ البيانات تلقائياً في Google Sheets عند المزامنة.');
            } else {
                Notification.error('❌ فشل حفظ البيانات المحلية');
            }
            return false;
        }
    },
