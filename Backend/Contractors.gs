...
function addContractorEvaluationToSheet(evaluationData) {
    try {
        ...
        if (!evaluationData.id) {
            evaluationData.id = generateSequentialId('CEV', sheetName, spreadsheetId);
        }
        if (!evaluationData.createdAt) {
            evaluationData.createdAt = new Date();
        }
        if (!evaluationData.updatedAt) {
            evaluationData.updatedAt = new Date();
        }
        if (!evaluationData.status) {
            evaluationData.status = 'قيد المراجعة';
        }
        
        // ✅ استخدام appendToSheet مع spreadsheetId للاتساق
        return appendToSheet(sheetName, evaluationData, spreadsheetId);
    } catch (error) {
