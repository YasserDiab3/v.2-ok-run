...
window.UI = {
            ...
            <div class="modal-content" style="max-width: 720px;">
                ...
                <div class="modal-body">
                    <form id="alert-form" class="space-y-4">
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">القنوات المستخدمة *</label>
                            <div class="flex flex-wrap gap-2" id="alert-channels-container">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-2">المنطقة المتأثرة *</label>
                            ...
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-2">فرق الاستجابة</label>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-2">التصعيد التلقائي (دقائق)</label>
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">تعليمات الاستجابة</label>
            ...
            <div class="modal-content" style="max-width: 720px;">
                ...
                <div class="modal-body">
                    <form id="plan-form" class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-2">الفريق المسؤول</label>
                            ...
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-2">آخر اختبار</label>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-2">اسم مسؤول الخطة</label>
    ...
    async showMainApp() {
        ...
        // ✅ إصلاح: إخفاء شاشة الدخول أولاً قبل أي شيء آخر
        // هذا يضمن عدم ظهور شاشة الدخول حتى لو كان هناك تأخير بسيط
        const loginScreen = document.getElementById('login-screen');
        const mainApp = document.getElementById('main-app');
        if (loginScreen) {
            loginScreen.style.display = 'none';
            // إزالة أي classes قد تظهر الشاشة
        ...
        if (typeof InactivityManager !== 'undefined' && !AppState.isPageRefresh) {
            ...
            // في حالة إعادة التحميل، ننتظر قليلاً ثم نفعّل النظام
            setTimeout(() => {
        ...
        (async () => {
            try {
                ...
                // تحميل Dashboard مباشرة إذا كان القسم المطلوب هو dashboard
                if (sectionToShow === 'dashboard' && typeof Dashboard !== 'undefined' && Dashboard.load) {
                    const loadResult = Dashboard.load();
                    if (loadResult && typeof loadResult.catch === 'function') {
                        loadResult.catch(err => Utils.safeWarn('⚠ فشل تحميل لوحة التحكم:', err));
                    }
                }

                // تحديث جميع البيانات المرئية
    ...
    showSection(sectionName) {
        ...
        // إذا كانت إعادة تحميل، نحمّل بصمت
        const isRefresh = AppState.isPageRefresh;
        setTimeout(() => {
            this.loadSectionData(sectionName, isRefresh);

            // إعادة إضافة أيقونات التنقل بعد تحميل محتوى الموديول
            // نستخدم تأخيرات متعددة لضمان إضافة الأيقونات بعد عرض المحتوى
            if (sectionName !== 'dashboard') {
                const addIconsAfterLoad = () => {
                    ...
                    if (section) {
                        ...
                        this.removeNavigationIcons(section);
                        // إضافة الأيقونات بعد تحميل المحتوى
                        this.addNavigationIcons(section, sectionName);
                    }
                };

                // محاولات متعددة لضمان إضافة الأيقونات بعد عرض المحتوى
                // نزيد التأخير لضمان تحميل المحتوى أولاً
    ...
    loadSectionData(sectionName, silent = false) {
        ...
        // تنظيف الموديول السابق قبل تحميل موديول جديد (لمنع تسريبات الذاكرة)
