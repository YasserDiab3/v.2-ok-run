...
function saveToSheet(sheetName, data, spreadsheetId = null) {
    try {
        ...
        const processDataItem = function(item) {
            ...
            // ✅ تطبيع خاص بورقة PTWRegistry لضمان عدم وجود JSON أو objects
            if (sheetName === 'PTWRegistry') {
                // تحويل permitType إلى string دائماً (وليس array)
                ...
                // التأكد من أن permitTypeDisplay هو string
                if (processed.permitTypeDisplay !== undefined) {
                    processed.permitTypeDisplay = normalizeTextValue(processed.permitTypeDisplay);
                }
                
                // التأكد من أن جميع الحقول النصية هي strings
                const textFields = ['requestingParty', 'location', 'sublocation', 'authorizedParty', 'workDescription', 'supervisor1', 'supervisor2', 'status', 'closureReason'];
                textFields.forEach(field => {
                    if (processed[field] !== undefined && processed[field] !== null) {
                        if (typeof processed[field] === 'object') {
                            // إذا كان object، استخراج name أو email أو id
                ...
                // التأكد من أن locationId و sublocationId هما strings أو null
                if (processed.locationId !== undefined && processed.locationId !== null) {
                    processed.locationId = String(processed.locationId).trim() || null;
                }
                if (processed.sublocationId !== undefined && processed.sublocationId !== null) {
                    processed.sublocationId = String(processed.sublocationId).trim() || null;
                }
                
                // التأكد من أن totalTime هو string أو number
                if (processed.totalTime !== undefined && processed.totalTime !== null) {
                    if (typeof processed.totalTime === 'object') {
                        processed.totalTime = String(processed.totalTime);
                    } else {
                        processed.totalTime = String(processed.totalTime).trim();
                    }
                }
            }
                    ...
                    if (typeof processedValue === 'object' && processedValue !== null) {
                        ...
                        try {
                            if (Object.prototype.toString.call(processedValue) === '[object Date]' && !isNaN(processedValue.getTime())) {
                                ...
                                if (timeFields.includes(cleanHeader)) {
