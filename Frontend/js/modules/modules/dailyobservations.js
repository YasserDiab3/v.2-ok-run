...
const DailyObservations = {
    ...
    normalizeRecord(record = {}) {
        ...
        return {
            id: record.id || record.observationId || '',
            isoCode: record.isoCode || record.code || '',
            siteId,
    ...
    async handleSubmit(form, editId = null, modal) {
        ...
        const recordId = editId || Utils.generateSequentialId('DOB', AppState.appData.dailyObservations || []);
        const isoCode = editId
            ? (existingRecord?.isoCode || generateISOCode('OBS', AppState.appData.dailyObservations))
            : generateISOCode('OBS', AppState.appData.dailyObservations);
    ...
    mapImportedObservationRow(row) {
        ...
        const recordId = Utils.generateSequentialId('DOB', AppState.appData.dailyObservations || []);
        const iso = isoCode || generateISOCode('OBS', AppState.appData.dailyObservations);
        const now = new Date().toISOString();
