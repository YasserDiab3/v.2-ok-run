    ...
    calculateStatistics() {
        ...
        employees.forEach(emp => {
            ...
            if (genderCheck.isMale) {
                ...
                unknownCount++;
                // تسجيل تشخيصي للقيم غير المعروفة (للتشخيص فقط)
                if (typeof console !== 'undefined' && console.warn) {
                    const originalValue = String(emp.gender || '');
                    console.warn(`⚠️ [Employees] قيمة النوع غير معروفة: "${originalValue}" (مطبيع: "${genderCheck.normalized}") للموظف: ${emp.name || emp.employeeNumber || emp.id || 'غير معروف'}`);
                }
            }
        });
        
        // تسجيل تشخيصي للإحصائيات (للتشخيص فقط - يمكن حذفه في الإنتاج)
        if (typeof console !== 'undefined' && console.log && unknownCount > 0) {
            console.log(`📊 [Employees] إحصائيات النوع - ذكر: ${maleCount}, أنثى: ${femaleCount}, غير معروف/فارغ: ${unknownCount} من ${total}`);
        }
