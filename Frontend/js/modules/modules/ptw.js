...
const PTW = {
            ...
            if (mapContent) {
                ...
                this.mapInitTimeout = setTimeout(() => {
                    ...
                    this.initMap().catch(error => {
                        Utils.safeError('❌ Map init error:', error);
                    });
                        
                        // Force map resize after initialization
                        setTimeout(() => {
                            if (this.mapInstance && this.currentTab === 'map') {
                                if (this.mapType === 'leaflet' && typeof L !== 'undefined' && this.mapInstance.invalidateSize) {
                                    this.mapInstance.invalidateSize();
                                    Utils.safeLog('✅ تم تحديث حجم الخريطة بعد التبديل للتبويب');
                                } else if (this.mapType === 'google' && typeof google !== 'undefined' && google.maps && this.mapInstance) {
                ...
                <tr data-registry-id="${entry.id}">
                    ...
                    <td>
                        <div class="flex items-center gap-1 flex-wrap">
                            ${entry.permitId && entry.permitId.startsWith('PTW_MANUAL') ? `
    ...
    async initMap() {
        ...
        try {
                    ...
                    setTimeout(() => {
                        if (this.mapInstance) {
                            if (this.mapType === 'leaflet' && this.mapInstance.invalidateSize) {
                                this.mapInstance.invalidateSize();
                            } else if (this.mapType === 'google' && google.maps && google.maps.event) {
                                google.maps.event.trigger(this.mapInstance, 'resize');
                            }
                        }
                    }, 300);
            ...
            Utils.safeError('❌ خطأ في تهيئة الخريطة:', error);
            if (loadingDiv) loadingDiv.style.display = 'none';
            if (errorDiv) {
                errorDiv.classList.remove('hidden');
                const errorMsg = errorDiv.querySelector('#ptw-map-error-message');
    ...
    async initLeafletMap(container, defaultCoords) {
        ...
        try {
            ...
            // قائمة مصادر الخرائط الفضائية بالترتيب (من الأكثر استقراراً إلى البدائل)
            // ✅ نبدأ بمصادر مستقرة (CartoDB) لتجنب أخطاء 503 من ArcGIS
            // استخدام مصادر متعددة لضمان عمل الخريطة الفضائية حتى عند فشل أحدها
            const satelliteSources = [
                // 1. CartoDB Dark - مصدر مستقر جداً، بدون أخطاء 503
                {
                    name: 'carto-dark',
                ...
                // 2. CartoDB Voyager - مصدر مستقر، بدون مشاكل
                {
                    name: 'carto-voyager',
                ...
                // 3. CartoDB Positron - مصدر مستقر إضافي
                {
                    name: 'carto-positron',
                ...
                // 4. ArcGIS - احتياطي فقط (قد يظهر أخطاء 503)
                {
                    name: 'esri-services',
                    url: 'https://services.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}',
                    attribution: '© <a href="https://www.esri.com/">Esri</a>, Maxar, GeoEye, Earthstar Geographics, CNES/Airbus DS, USDA, USGS, AeroGRID, IGN, IGP'
                }
            ];
            ...
            const handleSatelliteTileError = (error, tile) => {
                ...
                if (tile && tile.el) {
                    ...
                    // إزالة معالجات الأخطاء لمنع عرض الأخطاء
                    try {
                        tile.el.onerror = function() { return false; };
                    } catch(e) {}
                }
                
                // إذا تجاوزنا 3 أخطاء متتالية من نفس المصدر، ننتقل تلقائياً لمصدر بديل
                // هذا يضمن تبديل سريع عند فشل المصدر
                if (currentErrors >= 3 && this.mapInstance && this.leafletLayers.satellite) {
                    const nextSourceIndex = (currentSatelliteSourceIndex + 1) % satelliteSources.length;
                    
                    // التحقق من نجاحات المصدر التالي قبل التبديل
                    ...
                    // التبديل فقط إذا كان المصدر التالي أفضل (نجاحات أكثر أو أخطاء أقل)
                    // أو إذا كان المصدر الحالي فشل تماماً
                    if (nextSourceIndex !== currentSatelliteSourceIndex && 
                        (currentErrors > 5 || nextSourceSuccess > nextSourceErrors || nextSourceErrors < 2)) {
                        
                        try {
                            const isActive = this.mapInstance.hasLayer(this.leafletLayers.satellite);
                            ...
                            this.leafletLayers.satellite = createSatelliteLayer(currentSatelliteSourceIndex);
                            
                            // إعادة إضافة الطبقة إذا كانت مرئية
                            if (wasVisible && this.mapInstance) {
            ...
            this.leafletLayers.terrain.on('tileerror', (error, tile) => {
                ...
                // Leaflet سيعيد المحاولة تلقائياً
                if (tile && tile.el) {
                    // إخفاء الخطأ بصمت
                    tile.el.style.opacity = '0.3';
                    // إعادة المحاولة بعد تأخير قصير
                    setTimeout(() => {
                        if (tile && tile.el && this.leafletLayers.terrain) {
            ...
            Utils.safeError('❌ خطأ في تهيئة خريطة Leaflet:', error);
            Utils.safeError('❌ تفاصيل الخطأ:', error.stack || error.message);
    ...
    updateMapMarkers() {
        ...
        if (this.mapMarkers.length > 0) {
            try {
                if (this.mapType === 'google' && this.mapInstance) {
                    ...
                    if (this.mapMarkers.length === 1 && this.mapInstance.setZoom) {
    ...
    viewRegistryDetails(permitId) {
        ...
        // التحقق إذا كان تصريح يدوي (لا يوجد item)
        const isManualPermit = permitId.startsWith('PTW_MANUAL');
        ...
        if (isManualPermit && registryEntry && !item) {
    ...
    collectFormDataForPrint() {
        ...
        return {
            ...
            requestingParty: document.getElementById('ptw-requestingParty')?.value || '',
            authorizedParty: (() => {
    ...
    async saveManualPermitEntry(modal, entryId = null) {
        try {
            ...
            // جمع أنواع التصريح المحددة من checkboxes
            const selectedPermitTypes = Array.from(modal.querySelectorAll('input[name="manual-permit-type"]:checked'))
                .map(cb => cb.value);

            if (selectedPermitTypes.length === 0) {
                Notification.warning('يرجى اختيار نوع تصريح واحد على الأقل');
                return;
            }
            ...
            if (typeof GoogleIntegration !== 'undefined' && GoogleIntegration.autoSave) {
                await GoogleIntegration.autoSave('PTW', AppState.appData.ptw).catch(error => {
            ...
            // 5. تحديث الخريطة إذا كانت مرئية (للتأكد من عرض التصريح الجديد/المحدث)
            const mapContent = document.getElementById('ptw-map-content');
            if (mapContent && this.currentTab === 'map' && mapContent.style.display !== 'none') {
                // إعادة تحميل الخريطة لتحديث العلامات
                if (this.mapInstance && typeof this.initMap === 'function') {
                    setTimeout(() => {
                        if (this.currentTab === 'map') {
                            this.initMap().catch(error => {
                                Utils.safeWarn('⚠️ خطأ في تحديث الخريطة:', error);
                            });
                        }
                    }, 300);
                }
            ...
            // 6. تحديث KPIs في جميع التبويبات
            this.updateKPIs();
    ...
    async deleteManualPermitEntry(entryId) {
        ...
        try {
            ...
            if (!entry.permitId || !entry.permitId.startsWith('PTW_MANUAL')) {
    ...
    async renderList() {
        ...
        const allItems = Array.from(allPermitsMap.values());
        const openCount = allItems.filter(p => p && p.status !== 'مغلق' && p.status !== 'مرفوض').length;
        const closedCount = allItems.filter(p => p && p.status === 'مغلق').length;
        const totalCount = allItems.length;
        ...
        allItems.forEach(item => {
            ...
            if (item.status === 'مغلق' || item.status === 'مرفوض') {
                ...
                <div class="card-header">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-4">
                            <input type="text" id="ptw-search" class="form-input" style="max-width: 300px;" placeholder="البحث...">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                ...
                                <div>
                                    ...
                                    <input type="text" id="ptw-requestingParty" class="form-input transition-all focus:ring-2 focus:ring-blue-200"
                                        value="${escapeHTML(ptwData?.requestingParty)}" placeholder="الجهة الطالبة للتصريح">
                                </div>
                                <div class="md:col-span-3">
                                    <label class="block text-sm font-bold text-gray-700 mb-2">المعدة / المكينة / العملية</label>
    ...
    async handleSubmit(e) {
        ...
        const formData = {
            ...
            requestingParty: document.getElementById('ptw-requestingParty')?.value.trim() || '',
...
<body>
    ...
    switchMapType(type) {
        ...
        if (this.mapType === 'google') {
            ...
            // Leaflet - استخدام requestAnimationFrame لتأجيل العمليات وتقليل تأثير extensions
            requestAnimationFrame(() => {
                try {
                    // إزالة جميع الطبقات
                    try {
    ...
    filterItems(searchTerm = '', statusFilter = '') {
        ...
        const permitsFromRegistry = (this.registryData || []).map(registryEntry => {
            ...
            return {
                ...
                approvals: [], // التصاريح من السجل قد لا تحتوي على موافقات
        ...
        let filtered = Array.from(allPermitsMap.values());
    ...
    renderAnalysisContent() {
        ...
        const approvedPermits = allPermits.filter(p => p.status === 'موافق عليه').length;
        const pendingPermits = allPermits.filter(p => p.status === 'قيد المراجعة').length;
        const rejectedPermits = allPermits.filter(p => p.status === 'مرفوض').length;
                        ...
                        <div class="card-body">
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-2xl font-bold text-blue-600">${totalPermits}</p>
                                </div>
                        ...
                        <div class="card-body">
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-2xl font-bold text-green-600">${approvedPermits}</p>
                                </div>
                        ...
                        <div class="card-body">
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-2xl font-bold text-yellow-600">${pendingPermits}</p>
                                </div>
                        ...
                        <div class="card-body">
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-2xl font-bold text-red-600">${rejectedPermits}</p>
                                </div>
    ...
    setupAnalysisEventListeners() {
        ...
        if (addBtn) {
            // إنشاء نسخة جديدة من الزر لإزالة المستمعين السابقين
        ...
        // إضافة مستمعين لأزرار التعديل والحذف في الجدول
        setTimeout(() => {
