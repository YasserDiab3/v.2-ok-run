...
const Clinic = {
            ...
            <div class="clinic-card-edit-form border rounded-lg p-4 bg-white" data-card-id="${safe(card.id)}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="title" class="form-input" value="${safe(card.title)}">
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-2">الأيقونة (FontAwesome class)</label>
                        <input type="text" name="icon" class="form-input" value="${safe(card.icon || 'fas fa-info-circle')}">
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium mb-2">الوصف</label>
                        <input type="text" name="description" class="form-input" value="${safe(card.description)}">
                    </div>
                    <div>
                    ...
                    <div>
                        <label class="block text-sm font-medium mb-2">نوع الكرت</label>
                        <select name="mode" class="form-input clinic-card-mode">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 clinic-card-metric-wrap" style="display:${card.mode === 'metric' ? 'grid' : 'none'}">
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium mb-2">المؤشر</label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4 clinic-card-field-wrap" style="display:${card.mode === 'countByField' ? 'grid' : 'none'}">
                    <div>
                        ...
                        <select name="dataset" class="form-input">
                            ${datasets.map(d => `<option value="${d.value}" ${card.dataset === d.value ? 'selected' : ''}>${safe(d.label)}</option>`).join('')}
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-2">الحقل</label>
                        <input type="text" name="field" class="form-input" placeholder="مثال: status / reason" value="${safe(card.field)}">
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-2">القيمة (اختياري)</label>
                        <input type="text" name="fieldValue" class="form-input" placeholder="إذا تُرك فارغًا = أي قيمة" value="${safe(card.fieldValue)}">
                    </div>
                </div>
            </div>
        `;
        ...
        modal.innerHTML = `
            <div class="modal-content" style="max-width: 760px;">
                ...
                <div class="modal-body space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="text-sm font-semibold text-gray-600">اسم المصاب</label>
                            <p class="text-gray-800">${Utils.escapeHTML(name)}</p>
                        </div>
                        <div>
                            <label class="text-sm font-semibold text-gray-600">الإدارة / القسم</label>
                        ...
                        ` : ''}
                        <div>
                            <label class="text-sm font-semibold text-gray-600">تاريخ الإصابة</label>
                            <p class="text-gray-800">${this.formatDate(record.injuryDate, true)}</p>
                        </div>
                        <div>
                            <label class="text-sm font-semibold text-gray-600">نوع الإصابة</label>
                        ...
                        <div class="md:col-span-2">
                            <label class="text-sm font-semibold text-gray-600">مكان الإصابة</label>
                            <p class="text-gray-800">${Utils.escapeHTML(record.injuryLocation || '')}</p>
                        </div>
                        <div class="md:col-span-2">
                            <label class="text-sm font-semibold text-gray-600">وصف الإصابة</label>
                        ...
                        ${record.treatment ? `
                            <div class="md:col-span-2">
                                <label class="text-sm font-semibold text-gray-600">العلاج</label>
                                <p class="text-gray-800 whitespace-pre-line">${Utils.escapeHTML(record.treatment || '')}</p>
                            </div>
                        ` : ''}
                    </div>
                    <div>
                        <label class="text-sm font-semibold text-gray-600 mb-2 block">المرفقات</label>
                        <div class="space-y-2">
                            ${attachmentsHtml}
                        </div>
                    </div>
        ...
        modal.innerHTML = `
            <div class="modal-content" style="max-width: 720px;">
                ...
                <div class="modal-body space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="text-sm font-semibold text-gray-600">اسم الدواء</label>
                            <p class="text-gray-800">${Utils.escapeHTML(record.name || '')}</p>
                        </div>
                        <div>
                            <label class="text-sm font-semibold text-gray-600">نوع الدواء</label>
                            <p class="text-gray-800">${Utils.escapeHTML(record.type || '')}</p>
                        </div>
                        <div>
                            <label class="text-sm font-semibold text-gray-600">الاستخدام</label>
                            <p class="text-gray-800">${Utils.escapeHTML(record.usage || record.notes || '—')}</p>
                        </div>
                        <div>
                            <label class="text-sm font-semibold text-gray-600">تاريخ الشراء</label>
                            <p class="text-gray-800">${this.formatDate(record.purchaseDate)}</p>
                        </div>
                        <div>
                            <label class="text-sm font-semibold text-gray-600">تاريخ انتهاء الصلاحية</label>
                        ...
                        <div>
                            <label class="text-sm font-semibold text-gray-600">الرصيد</label>
                            <p class="text-gray-800 font-semibold">${record.remainingQuantity ?? record.quantity ?? 0}</p>
                        </div>
                        <div>
                            <label class="text-sm font-semibold text-gray-600">المنصرف</label>
                        ...
                        <div>
                            <label class="text-sm font-semibold text-gray-600">موقع التخزين</label>
            ...
            <div class="modal-content" style="max-width: 740px;">
                ...
                <div class="modal-body space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        ` : ''}
                        <div>
                            <label class="text-sm font-semibold text-gray-600">الطبيب المعالج</label>
                            <p class="text-gray-800">${Utils.escapeHTML(doctor)}</p>
                        </div>
                        <div>
                            <label class="text-sm font-semibold text-gray-600">تاريخ البداية</label>
                            <p class="text-gray-800">${start}</p>
                        </div>
                        <div>
                            <label class="text-sm font-semibold text-gray-600">تاريخ النهاية</label>
                            <p class="text-gray-800">${end}</p>
                        </div>
                        <div>
                            <label class="text-sm font-semibold text-gray-600">عدد الأيام</label>
                            <p class="text-gray-800">${days}</p>
                        </div>
                    </div>
                    <div>
                        <label class="text-sm font-semibold text-gray-600">سبب الإجازة</label>
                        <p class="text-gray-800 whitespace-pre-line">${Utils.escapeHTML(record.reason || '')}</p>
                    </div>
                    ${record.medicalNotes ? `
                        <div>
                            <label class="text-sm font-semibold text-gray-600">ملاحظات طبية</label>
                            ...
                            <div class="mb-6 border-t pt-6">
                                ...
                                <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <div id="clinic-analysis-items-list" class="space-y-2 max-h-64 overflow-y-auto border rounded p-3">
                                            <!-- filled dynamically -->
                                        </div>
                                    </div>
                                    <div>
                        ...
                        <div class="form-section" style="background: linear-gradient(135deg, #f5f7fa 0%, #c3cfe2 100%); padding: 25px; border-radius: 15px; box-shadow: 0 8px 16px rgba(0,0,0,0.1);">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div style="background: white; padding: 15px; border-radius: 10px; border: 2px solid #667eea;">
                                    <label style="display: flex; align-items: center; gap: 8px; color: #667eea; font-weight: 600; font-size: 13px; margin-bottom: 8px;">
                        ...
                        <div class="form-section" style="background: linear-gradient(135deg, #ffecd2 0%, #fcb69f 100%); padding: 25px; border-radius: 15px; box-shadow: 0 8px 16px rgba(0,0,0,0.1);">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div style="background: white; padding: 15px; border-radius: 10px; border: 2px solid #fc6c85;">
                                    <label style="display: flex; align-items: center; gap: 8px; color: #fc6c85; font-weight: 600; font-size: 13px; margin-bottom: 8px;">
                        ...
                        <div class="form-section" style="background: linear-gradient(135deg, #a1c4fd 0%, #c2e9fb 100%); padding: 25px; border-radius: 15px; box-shadow: 0 8px 16px rgba(0,0,0,0.1);">
                            ...
                            <div class="space-y-4">
                                <div style="background: white; padding: 15px; border-radius: 10px; border: 2px solid #4facfe;">
                                    <label style="display: flex; align-items: center; gap: 8px; color: #4facfe; font-weight: 600; font-size: 13px; margin-bottom: 8px;">
    ...
    exportDispensedMedicationsToExcel(medications) {
        ...
        if (typeof Reports !== 'undefined' && Reports.exportToExcel) {
            Reports.exportToExcel(data, `سجل_الأدوية_المنصرفة_${new Date().toISOString().slice(0, 10)}`);
        } else {
            Notification?.error?.('وظيفة التصدير غير متاحة');
        }
    },
