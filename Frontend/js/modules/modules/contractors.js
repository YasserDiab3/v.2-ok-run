...
const Contractors = {
    ...
    ensureData() {
        ...
        // ✅ إصلاح: التأكد من وجود جميع المصفوفات (مثل العيادة الطبية)
        ...
        // ✅ إصلاح: حفظ البيانات في AppState مباشرة (مثل العيادة الطبية)
        AppState.appData = data;
        
        // ✅ إصلاح: حفظ البيانات في localStorage لضمان ثباتها (مثل العيادة الطبية)
        if (typeof window.DataManager !== 'undefined' && window.DataManager.save) {
            window.DataManager.save();
        }
    },
    ...
    async submitApprovalRequest(modal, attachments = [], onStart = null, onError = null) {
        try {
            ...
            // ✅ تحديث العرض مباشرة بعد الحفظ المحلي
            this.refreshApprovalRequestsSection();
            
            // ✅ المزامنة مع Backend في الخلفية (بدون انتظار)
    ...
    async approveRequest(requestId, requestCategory = 'approval') {
        ...
        try {
            ...
            // ✅ التحقق من أن request موجود وأن ID يبدأ بـ "CAR_"
            if (!request) {
                Loading.hide();
            ...
            // ✅ التحقق النهائي: التأكد من أن ID يبدأ بـ "CAR_"
            if (request.id && !request.id.startsWith('CAR_')) {
                Loading.hide();
                Notification.error('خطأ: الطلب لا يزال قيد المزامنة أو لم يتم حفظه بشكل صحيح. يرجى المحاولة لاحقاً.');
                Utils.safeError('❌ خطأ: الطلب يحتوي على ID غير صحيح. requestId=' + request.id + ' (يجب أن يبدأ بـ CAR_)');
                return;
            }
            
            // ✅ استخدام ID الصحيح (CAR_...) للاعتماد
        ...
        return `
            <div class="content-card border-2 border-indigo-200 rounded-xl shadow-xl overflow-hidden">
                ...
                <div class="card-body p-6 bg-gray-50">
                    ...
                    ${this.renderDetailedContractorAnalysis(contractors, approvedContractors, evaluations, violations)}
                </div>
            </div>
        `;
    },
                    ...
                    <div class="overflow-x-auto">
                        <table class="data-table w-full">
                            <thead class="bg-indigo-100">
                                <tr>
                                    <th class="px-6 py-4 text-right font-bold text-indigo-900 border-b border-indigo-200">اسم المقاول</th>
                                    <th class="px-6 py-4 text-center font-bold text-indigo-900 border-b border-indigo-200">نوع الخدمة</th>
                                    <th class="px-6 py-4 text-center font-bold text-indigo-900 border-b border-indigo-200">حالة العقد</th>
                                    <th class="px-6 py-4 text-center font-bold text-indigo-900 border-b border-indigo-200">التقييمات</th>
                                    <th class="px-6 py-4 text-center font-bold text-indigo-900 border-b border-indigo-200">متوسط التقييم</th>
                                    <th class="px-6 py-4 text-center font-bold text-indigo-900 border-b border-indigo-200">المخالفات</th>
                                    <th class="px-6 py-4 text-center font-bold text-indigo-900 border-b border-indigo-200">عالية</th>
                                    <th class="px-6 py-4 text-center font-bold text-indigo-900 border-b border-indigo-200">معدل الحل</th>
                                    <th class="px-6 py-4 text-center font-bold text-indigo-900 border-b border-indigo-200">الإجراءات</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-100">
                                    ...
                                    <tr class="hover:bg-indigo-50 transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}">
                                        <td class="px-6 py-4">
                                            <div class="flex items-center">
                                                ...
                                                <div>
                                                    <strong class="text-gray-800 font-semibold block">${Utils.escapeHTML(contractor.name || '')}</strong>
                                                    <span class="text-xs text-gray-500">
                                                        <span class="badge badge-${contractor.status === 'نشط' ? 'success' : 'danger'} text-xs">
                                                            ${contractor.status || '-'}
                                                        </span>
                                                    </span>
                                                </div>
                                        ...
                                        <td class="px-6 py-4 text-center">
                                            <span class="inline-flex items-center px-3 py-1 rounded-full text-sm bg-blue-100 text-blue-700">
                                                ${Utils.escapeHTML(contractor.serviceType || '-')}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 text-center">
    ...
    viewContractorAnalytics(contractorId) {
        ...
        const evaluations = (AppState.appData.contractorEvaluations || []).filter(e => 
            e.contractorId === contractorId || e.contractorName === contractor.name
        );
            ...
            <div class="modal-content" style="max-width: 1000px; max-height: 90vh; overflow-y: auto;">
                <div class="modal-header bg-gradient-to-r from-indigo-600 to-purple-600 text-white">
                    <div class="flex items-center">
                        ...
                        <div>
                            <h2 class="modal-title text-xl font-bold">تحليل مفصل: ${Utils.escapeHTML(contractor.name || '')}</h2>
                            <p class="text-sm text-indigo-100 mt-1">${Utils.escapeHTML(contractor.serviceType || '')}</p>
                        </div>
                    </div>
                ...
                <div class="modal-body p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                        <div class="bg-gradient-to-br from-blue-50 to-blue-100 border-2 border-blue-300 rounded-xl p-5 shadow-md">
                            ...
                            <p class="text-3xl font-bold text-blue-700">${evaluations.length}</p>
                        </div>
